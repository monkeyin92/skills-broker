...
- [x] **LOOP-01**: Operator can see from repo-owned surfaces that `website QA` is the single first step, with `web markdown` and `social markdown` presented as the second and third proven loops in that order
- [x] **LOOP-02**: Installed host shells and repo docs can direct the operator from `website QA` into the next proven loop without telling the host to choose a concrete downstream winner
- [x] **LOOP-03**: Operator can tell what the recommended next loop is after a successful `website QA` proof without reconstructing the hierarchy from multiple docs or traces
