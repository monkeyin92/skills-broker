...
test("cli includes a downstream local skill path when broker-owned skills are reused across hosts", async () => {
  const runtimeDirectory = await mkdtemp(join(tmpdir(), "skills-broker-cli-handoff-"));
